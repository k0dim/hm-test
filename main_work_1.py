...
def get_doc_owner_name(user_doc_number): # test_p
...
def delete_doc(user_doc_number): # test_d
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
...
def get_doc_shelf(user_doc_number): # test_s
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            owner_name = get_doc_owner_name(user_doc_number = get_num_doc())
            ...
            directory_number = get_doc_shelf(user_doc_number = get_num_doc())
            print('Документ находится на полке номер {}'.format(directory_number))
        elif user_command == 'a':
            print('Добавление нового документа:')
            dict_info, num_shelh = request_for_doc()
            new_doc_shelf_number = add_new_doc(dict_info, num_shelh)
            print('\nНа полку "{}" добавлен новый документ:'.format(new_doc_shelf_number))
        elif user_command == 'd':
            doc_number, deleted = delete_doc(user_doc_number = get_num_doc())
            if deleted:
